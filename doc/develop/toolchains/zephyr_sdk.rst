...
All three contain the host tools; The ``gnu`` bundle contains also the GNU toolchains (i.e.
Binutils, GCC and GDB) for all supported architectures; The ``llvm`` bundle contains the LLVM/Clang
toolchain; And the ``minimal`` one does not contain either.
