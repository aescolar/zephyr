...
static const char default_flash_file_path[] = DEFAULT_FLASH_FILE_PATH;
static bool flash_erase_at_start;
static bool flash_rm_at_exit;
static bool flash_on_ram;
#else
...
static int flash_mock_init(const struct device *dev)
{
	...
	if (flash_on_ram == true) {
		mock_flash = (uint8_t *)malloc(FLASH_SIMULATOR_FLASH_SIZE);
		if (mock_flash == NULL) {
			posix_print_warning("Could not allocate flash in Linux process heap %s\n",
					    strerror(errno));
			return -EIO;
		}
	} else {
	...
	if ((flash_erase_at_start == true) || (flash_on_ram == true) || (f_stat.st_size == 0)) {
