...
static int settings_custom_load(struct settings_store *cs, const struct settings_load_arg *arg)
{
	...
	if (fp == NULL) {
		LOG_INF("Settings file doesn't exist yet, will be created on first save");
		return -1;
	}
...
static const char settings_file_suffix[] = "settings_client";

int settings_backend_init(void)
{
	snprintf(setting_file, sizeof(setting_file), "%s_%s.log", get_simid(), settings_file_suffix);
