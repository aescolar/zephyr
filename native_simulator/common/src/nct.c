 ...
 *
 * To create a new hosted thread, the hosted OS shall call nct_new_thread().
 * To swap to a thread nct_swap_threads(), and to terminate a thread
 * nct_abort_thread().
 * The hosted OS can optionally use nct_main_thread_start() to swap
 * to the "first thread".
 *
 *
 * Internal design:
...
struct threads_table_el {
	struct te_status_t *ts_status; /* Pointer to the overall status of the threading emulator instance */
 ...
 *   this_th_nbr: Identifier of the thread the hosted OS wants to swap out
 *   	Note: the call must be done from the thread that is being swapped out.
 */
void nct_swap_threads(void *this_arg, int next_allowed_thread_nbr, int this_th_nbr)
{
 ...
 *  It will block the pthread until this thread is swapped in
 */
static void *nct_thread_starter(void *arg_el)
{
...
static int ttable_get_empty_slot(struct te_status_t *this)
{
	...
	/* Clear table piece */
	(void)memset(new_chunk, 0, NCT_ALLOC_CHUNK_SIZE * sizeof(struct threads_table_el));
 ...
 * with nct_new_thread() is swapped in (from inside that thread context)
 *
 */
 ...
 * convoluted cases. As a call to this function can come from an hosted OS
 ...
 * *  self       : Should be true (!=0) if the thread is cancelling itself.
 */
void nct_abort_thread(void *this_arg, int thread_idx, int self)
{
	struct te_status_t *this = (struct te_status_t *)this_arg;
 ...
 * developers.
 *
 * Background: This arch creates a pthread as soon as the hosted kernel creates
