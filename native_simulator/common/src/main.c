 ...
 *
 * The native simulator basic principle of operation is:
 ...
 *  The execution is decoupled from the underlying host and its
 *  peripherals (unless set otherwise).
 *  In general, time in the native simulator is simulated.
 *
 * But, the native simulator can also be linked if desired to the underlying host,
 * e.g.:For ex. you can use the provided Ethernet TAP driver, or a host BLE controller.
 * In this case, the no-indeterminism principle is lost. Runs of the native simulator
...
void nsi_exit(int exit_code)
{
	 ...
	 */
	nsif_cpu0_cleanup();
	nsi_run_tasks(NSITASK_ON_EXIT_LEVEL);
	hwm_cleanup();
