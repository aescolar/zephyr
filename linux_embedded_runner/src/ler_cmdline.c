...
void ler_add_command_line_opts(struct args_struct_t *args)
{
	...
	if (used_args + count >= args_aval) {
		...
		if (new_args_struct == NULL) {
			ler_print_error_and_exit("Could not allocate memory");
		} else {
...
void ler_add_testargs_option(void)
{
	...
	ler_add_command_line_opts(testargs_options);
}

static void print_invalid_opt_error(char *argv)
{
	ler_print_error_and_exit("Incorrect option '%s'. Did you misspell it?"
